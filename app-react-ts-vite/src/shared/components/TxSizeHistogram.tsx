...
const TxSizeHistogram = ({ transactions, onSelectTransactions }: { transactions: Transaction[], onSelectTransactions: (selectedTransactions: Transaction[]) => void }) => {
    const data: number[] = transactions.map(transaction => transaction.txsize);
    console.log(data)
    ...
    useEffect(() => {
        ...
        svg.selectAll("rect")
            ...
            .attr("width", function(d) { return Math.max(0, xScale(d.x1!) - xScale(d.x0!) - 6); })
        ...
        svg.call(d3.brushX<Transaction[]>()
            ...
            const selectedTransactions = transactions.filter(transaction => {
                return transaction.txsize >= x0 && transaction.txsize <= x1;
            });
    ...
    return (<svg ref={ref} width={550} height={200} id="txSizeHistogram"></svg>);
};

export default TxSizeHistogram;