...
const AddressesHistogram = ({ transactions, onSelectTransactions }: { transactions: Transaction[], onSelectTransactions: (selectedTransactions: Transaction[]) => void }) => {
    const data: number[] = transactions.map(transaction => transaction.inaddr + transaction.outaddr);
    console.log(data);
    ...
    useEffect(() => {
        ...
        svg.call(d3.brushX<Transaction[]>()
            ...
            const selectedTransactions = transactions.filter(transaction => {
                const numAddresses = transaction.inaddr + transaction.outaddr;
                return numAddresses >= x0 && numAddresses <= x1;
            });
    ...
    return (<svg ref={ref} width={550} height={210} id="AddressesHistogram"></svg>);
};

export default AddressesHistogram;